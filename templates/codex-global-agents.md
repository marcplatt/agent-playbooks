...
- Human gates are limited to unresolved business meaning, operator-facing function review, exact-head merge release when policy requires it, exact mutation/transmission or other material external-effect authority, and HRM closure. Bounded read-only diagnostics are routine. Sign-in, MFA, credential unlock, private-value entry, and physical presence are operator actions, not decisions. Present the exact decision or action; never ask only “approve” or “proceed.”
- Keep the execution capsule stable across routine workflow transitions; mutable progress belongs in the derived session state. Bind later merge or external-effect permission as an exact authority grant instead of rotating the capsule.
- Keep the root orchestrator coordination-only. Use fresh worker contexts for implementation/correction, declared checks, and provider diagnostics/evidence, and guard each action online against role, liveness, context, and execution-mode budgets.
