...
module HrmSupervisor
  ...
  TERMINAL_REASON_CODE_BY_SCORECARD_REASON = {
    "semantic-readiness startup budget exceeded" => "semantic_readiness_startup_budget_exceeded",
    "worker-handoff startup budget exceeded" => "worker_handoff_startup_budget_exceeded",
    "worker-activation startup budget exceeded" => "worker_activation_startup_budget_exceeded",
    "worker activation after handoff budget exceeded" => "worker_activation_handoff_budget_exceeded",
    "late S0/S1 finding" => "late_interrupt_finding",
    "non-runtime unit budget exceeded" => "nonruntime_unit_budget_exceeded",
    "correction-round budget exceeded" => "correction_round_budget_exceeded",
    "duplicate conclusive CI" => "duplicate_conclusive_ci",
    "CI-minute budget exceeded" => "ci_minute_budget_exceeded",
    "no-material-progress budget exceeded" => "no_material_progress_budget_exceeded",
    "context budget exceeded" => "context_budget_exceeded",
    "context-redundancy budget exceeded" => "context_redundancy_budget_exceeded",
    "pre-expected-value context-compaction budget exceeded" => "context_compaction_budget_exceeded",
    "inline raw-log budget exceeded" => "inline_raw_log_budget_exceeded",
    "state-artifact echo budget exceeded" => "state_artifact_echo_budget_exceeded",
    "context scope escape" => "context_scope_escape",
    "undispositioned scope addition" => "undispositioned_scope_addition",
    "non-blocking scope absorbed" => "nonblocking_scope_absorbed",
    "liveness stop triggered" => "liveness_stop_triggered",
    "outcome or safety gate failed" => "outcome_or_safety_gate_failed",
    "aftercare pending" => "aftercare_pending",
    "worker protocol failure" => "worker_protocol_failure"
  }.freeze
  ...
  def validate_dispatch!(dispatch, capsule = nil)
    ...
    if capsule && (rc12?(capsule) || activation_protocol?(capsule))
      ...
      expected_order = if rc19?(capsule)
                         %w[
                           activate
                           ...
                           context
                           guard
                           result_or_fail_closed
                         ]
                       elsif worker_context_plane?(capsule)
                         %w[
                           activate
                           verify_dispatch_digest_without_dump
                           read_worker_context_pack
      ...
      unless dispatch.dig("protocol", "working_directory") == expected_working_directory &&
             ...
             (!activation_protocol?(capsule) || dispatch.dig("protocol", "one_shot_argument_encoding") ==
               "unpadded_base64url_canonical_json") &&
             (!activation_protocol?(capsule) || dispatch.dig("protocol", "context_report", "argument") ==
               "base64url_canonical_json") &&
             (!activation_protocol?(capsule) || dispatch.dig("protocol", "context_report", "stdin").nil?) &&
             (!activation_protocol?(capsule) || dispatch.dig("protocol", "result_contract") ==
      ...
      if activation_protocol?(capsule)
        ...
        unless awaiting_result == !lifecycle_sha256.nil?
          raise HrmExperiment::ValidationError,
                "#{capsule.dig('playbook_pin', 'kernel_version')} dispatch result lifecycle binding must " \
                "appear exactly while awaiting a worker result"
        end
      end
    end
    expected_hash = HrmExperiment.object_sha256(
  ...
  def validate_release(capsule_path, events_path)
    ...
    if worker_context_plane?(capsule)
  ...
  def record_context(capsule_path, events_path, role, report)
    ...
    unless supervisor_measured_context?(capsule)
      raise HrmExperiment::ValidationError,
            "the supervisor context command requires an rc.9 through rc.19 capsule"
    end
    with_run_lock(events_path, capsule) do
      events = load_and_validate_run(capsule, events_path)
      ...
      worker_context = if worker_context_plane?(capsule) && role == "builder"
  ...
  def handoff(capsule_path, events_path, now = Time.now.utc)
    ...
    with_run_lock(events_path, capsule) do
      ...
      if worker_context_plane?(capsule) && role == "builder"
  ...
  def activate(capsule_path, events_path, claim_id, dispatch_cursor, dispatch_sha256, now = Time.now.utc)
    ...
    unless activation_protocol?(capsule)
      raise HrmExperiment::ValidationError,
            "the supervisor activate command requires an rc.13 capsule or its rc.14-rc.19 successors"
    end
    with_run_lock(events_path, capsule) do
      events = load_and_validate_run(capsule, events_path)
      ensure_run_writable!(capsule, events)
      ...
      validate_worker_context_pack!(capsule, events, events_path) if worker_context_plane?(capsule) && assignment["role"] == "builder"
      append_prepared(capsule, events, events_path, {
  ...
  def result_contract(capsule_path, events_path, claim_id)
    capsule = HrmExperiment.load_yaml(capsule_path)
    unless rc19?(capsule)
      raise HrmExperiment::ValidationError, "the supervisor result-contract command requires rc.19"
    end
    with_run_lock(events_path, capsule) do
      events = load_and_validate_run(capsule, events_path)
      ensure_run_writable!(capsule, events)
      ...
      assignment = {
        ...
        "role" => details.fetch("worker_role")
      }
      validate_worker_claim_for_assignment!(
        capsule,
        events,
      ...
      contract = {
        ...
        "assigned_action" => assignment.fetch("action"),
        "assigned_role" => assignment.fetch("role"),
      ...
      if JSON.generate(contract).bytesize > MAX_RECEIPT_BYTES
        raise HrmExperiment::ValidationError, "rc.19 result contract exceeds #{MAX_RECEIPT_BYTES} bytes"
      end
      contract
    end
  end

  def worker_failure(capsule_path, events_path, claim_id, failure, now = Time.now.utc)
    capsule = HrmExperiment.load_yaml(capsule_path)
    unless rc19?(capsule)
      raise HrmExperiment::ValidationError, "the supervisor worker-failure command requires rc.19"
    end
    with_run_lock(events_path, capsule) do
      events = load_and_validate_run(capsule, events_path)
      ensure_run_writable!(capsule, events)
      unless failure.is_a?(Hash) && failure.keys.sort == %w[error_code stage] &&
             %w[protocol_command_failed result_payload_invalid].include?(failure["error_code"]) &&
             failure["stage"] == "result"
        raise HrmExperiment::ValidationError, "worker failure requires only an allowed error_code and stage"
      end
      guard = events.last
      unless guard&.dig("event_type") == "action_guard_passed"
        raise HrmExperiment::ValidationError, "worker failure requires the exact pending post-guard lifecycle"
      end
      ...
      append_prepared(capsule, events, events_path, {
        ...
        "occurred_at" => now.utc.iso8601,
        "role" => "orchestrator",
        "caused_by_sequence" => guard.fetch("sequence"),
        "details" => {
          ...
          "worker_claim_id" => claim_id,
          "material_progress" => false,
          "note" => "Supervisor stopped after a claim-bound #{failure.fetch('stage')} #{failure.fetch('error_code')}."
        }
      }, "worker-failure", capsule_path)
    end
  end
  ...
  def result(capsule_path, events_path, result_event, now = Time.now.utc)
    ...
    unless result_protocol?(capsule)
      raise HrmExperiment::ValidationError, "the supervisor result command requires an rc.11 through rc.19 capsule"
    end
    with_run_lock(events_path, capsule) do
      events = load_and_validate_run(capsule, events_path)
      ensure_run_writable!(capsule, events)
  ...
  def derive_rc13_result_event!(capsule, action, role, claim, domain, now)
    unless domain.is_a?(Hash)
      raise HrmExperiment::ValidationError, "rc.13-rc.19 result domain payload must be a JSON object"
    end
    binding = {
      "action" => action,
      "worker_role" => role,
    ...
    when "implement_frozen_slice"
      ...
      expected_keys << "output_artifact_sha256" if worker_context_plane?(capsule)
    ...
    when *RC13_GENERIC_COMPLETION_ACTIONS
      ...
      raise HrmExperiment::ValidationError,
            "rc.13-rc.19 has no action-specific result contract for #{action.inspect}"
    end
  end
  ...
  def require_exact_domain_keys!(domain, expected_keys, action)
    ...
    raise HrmExperiment::ValidationError,
          "rc.13-rc.19 #{action} result requires only domain keys #{expected_keys.sort.join(', ')}"
  end
  ...
  def result_protocol_prefix!(capsule, events, role)
    ...
    expansion_events = if worker_context_plane?(capsule)
  ...
  def derive_successor_capsule(predecessor, successor_path, session_id, execution_mode)
    ...
    successor.dig("budgets")["context_bytes_by_role"] = HrmExperiment::RC18_CONTEXT_BYTES_BY_ROLE if worker_context_plane?(successor)
    if (rc16?(successor) || worker_context_plane?(successor)) && %w[runtime_build production_observation].include?(execution_mode)
  ...
  def validate_worker_context_pack!(capsule, events, events_path)
    raise HrmExperiment::ValidationError, "worker-context packs require rc.18 or rc.19" unless worker_context_plane?(capsule)
  ...
  def expand_context(capsule_path, events_path, claim_id, request, now = Time.now.utc)
    capsule = HrmExperiment.load_yaml(capsule_path)
    raise HrmExperiment::ValidationError, "context expansion requires rc.18 or rc.19" unless worker_context_plane?(capsule)
  ...
  def rc14?(capsule)
    capsule.dig("playbook_pin", "kernel_version") == RC14_KERNEL_VERSION
  end
  ...
  def rc19?(capsule)
    capsule.dig("playbook_pin", "kernel_version") == RC19_KERNEL_VERSION
  end

  def worker_context_plane?(capsule)
    rc18?(capsule) || rc19?(capsule)
  end

  def activation_projection?(capsule)
    rc15?(capsule) || rc16?(capsule) || worker_context_plane?(capsule)
  end
  ...
  def result_protocol?(capsule)
    rc11?(capsule) || rc12?(capsule) || activation_protocol?(capsule)
  end

  def zero_normalization?(capsule)
    rc12?(capsule) || activation_protocol?(capsule)
  end
  ...
  def validate_worker_claim_for_assignment!(
    ...
    require_activation: activation_protocol?(capsule)
  )
    handoff = events.reverse.find do |event|
      %w[worker_handoff_started worker_result_received].include?(event["event_type"])
    end
    ...
    verified_stored_result_lifecycle!(capsule, events, events_path) if activation_protocol?(capsule) && !recompute_source
    if require_activation
      ...
      unless activated
        raise HrmExperiment::ValidationError,
              ...
              "or its rc.14-rc.19 successor protocol " \
  ...
  def activation_claim_validation(activation_details)
    claimed = activation_details["activation_dispatch_sha256"]
    observed = activation_details["observed_activation_dispatch_sha256"]
    valid_digest = lambda do |value|
      value.is_a?(String) && value.match?(/\A[0-9a-f]{64}\z/)
    end
    {
      "claimed_dispatch_sha256" => claimed,
      "observed_dispatch_sha256" => observed,
      "matched" => valid_digest.call(claimed) && valid_digest.call(observed) && claimed == observed,
      "cursor" => activation_details["activation_dispatch_cursor"]
    }
  end

  def validate_rc15_activation_evidence!(activation_details, expected_dispatch_sha256 = nil)
    validation = activation_claim_validation(activation_details)
    valid = validation["matched"] &&
            (expected_dispatch_sha256.nil? ||
              validation["claimed_dispatch_sha256"] == expected_dispatch_sha256)
    unless valid
      raise HrmExperiment::ValidationError,
            "rc.15/rc.16/rc.18/rc.19 worker_started requires matching claimed and observed activation dispatch digests " \
            "bound to the exact deterministic activation dispatch digest"
    end
    validation
  end
  ...
  def normalize_context_report!(report, capsule, role = nil)
    ...
    dependency_field = if worker_context_plane?(capsule) && role == "builder"
  ...
  def build_refresh(capsule, events, events_path, action, capsule_path, receipt_fields)
    ...
    if activation_projection?(capsule) && action == "activate"
      ...
      receipt["post_activation_dispatch"] = {
    ...
    elsif activation_protocol?(capsule) && action == "activate"
  ...
  def worker_launch_from_dispatch(capsule, dispatch, paths)
    ...
    if rc14?(capsule) || activation_projection?(capsule)
      launch.merge!(
        "launch_policy" => {
          "fork_turns" => "none",
          "root_wait" => "none_before_one_after",
          "steps" => %w[
            activate
            verify_dispatch_digest_without_dump
            bounded_source_materialization
        ...
        "one_shot_input" => {
          ...
          "context_required_keys" => %w[loaded_artifact_bytes_by_id turn_id]
        },
        "result_input" => rc13_result_input_contract(action, capsule)
      )
    elsif rc13?(capsule)
      launch.merge!(
        "launch_ready" => false,
  ...
  def compact_rc14_launch_receipt(receipt)
    retained = %w[
      action
      session_id
      ledger_cursor
  ...
  def rc13_result_input_contract(action, capsule)
    ...
    when "implement_frozen_slice"
      ...
      required_keys << "output_artifact_sha256" if worker_context_plane?(capsule)
      template = {
      ...
      template["output_artifact_sha256"] = "0" * 64 if worker_context_plane?(capsule)
      {
    ...
    when *RC13_GENERIC_COMPLETION_ACTIONS
      ...
      raise HrmExperiment::ValidationError,
            "rc.13-rc.19 has no action-specific result input contract for #{action.inspect}"
    end
  end

  def validate_receipt_size!(capsule, receipt)
    return true unless worker_context_plane?(capsule) || (zero_normalization?(capsule) && receipt["worker_launch"])

    bytes = JSON.generate(receipt).bytesize
    limit = worker_context_plane?(capsule) ? RC18_RECEIPT_TARGET_BYTES : MAX_RECEIPT_BYTES
    if bytes > limit
      raise HrmExperiment::ValidationError,
  ...
  def derive_next_action(capsule, events, state, scorecard, api_skill_coverage, attention)
    ...
    elsif activation_protocol?(capsule) && derive_routine_next_action(capsule, events) == "await_worker_result"
      "await_worker_result"
  ...
  def derive_dispatch_envelope(capsule, events, next_action, api_skill_coverage, paths, capsule_path)
    ...
    if worker_context_plane?(capsule) && action == "implement_frozen_slice" && role == "builder"
    ...
    loaded_artifact_budget, tool_output_reserve = if worker_context_plane?(capsule) && role &&
    ...
    dispatch = {
      ...
      "protocol" => {
        ...
        "context_command" => role ? [
          ...
          protocol_events_path,
          role,
          *(activation_protocol?(capsule) ? ["__BASE64URL_CANONICAL_JSON_CONTEXT__"] : [])
        ] : nil,
        ...
        "result_command" => action && result_protocol?(capsule) ? [
          ...
          protocol_capsule_path,
          protocol_events_path,
          *(activation_protocol?(capsule) ? ["__BASE64URL_CANONICAL_JSON_DETAILS__"] : [])
        ] : nil,
    ...
    if activation_protocol?(capsule)
      dispatch.fetch("protocol")["worker_execution_order"] = if rc19?(capsule)
                                                                  %w[
                                                                    activate
                                                                    read_result_contract
                                                                    verify_dispatch_digest_without_dump
  ...
  def terminal_reason_codes(state, scorecard)
    return [] if state.dig("terminal", "state") == "active"

    codes = Array(scorecard.dig("verdict", "reasons")).map do |reason|
      TERMINAL_REASON_CODE_BY_SCORECARD_REASON[reason]
    end.compact
    if scorecard.dig("verdict", "process_envelope") == "fail" && codes.empty?
      codes << "process_envelope_failed"
    end
    if codes.empty? && state.dig("terminal", "stop_reason")
      codes << "stop_#{state.dig('terminal', 'stop_reason')}"
    end
    if codes.empty?
      codes << "terminal_#{state.dig('terminal', 'state')}"
    end
    codes.uniq.first(MAX_TERMINAL_REASON_CODES)
  end

  def derive_operator_projection(capsule, state, attention, reason_codes = nil, process_blocked = false)
    ...
    summary = if activation_projection?(capsule) && terminal
                ...
                capsule.dig("hrm", "outcome").to_s.gsub(/\s+/, " ").strip
              end
  ...
  def usage
    <<~TEXT
      Usage:
        ...
        ruby scripts/hrm_supervisor.rb result-contract CAPSULE.yaml EVENTS.jsonl CLAIM
        ruby scripts/hrm_supervisor.rb result CAPSULE.yaml EVENTS.jsonl [BASE64URL_CANONICAL_JSON]
        ruby scripts/hrm_supervisor.rb worker-failure CAPSULE.yaml EVENTS.jsonl CLAIM BASE64URL_CANONICAL_JSON
  ...
  begin
    ...
    receipt = case command
              ...
              when "context"
                ...
                capsule = HrmExperiment.load_yaml(capsule_path)
                report = if HrmSupervisor.activation_protocol?(capsule)
              ...
              when "worker-failure"
                ...
                HrmSupervisor.worker_failure(
                  ...
                  events_path,
                  claim_id,
                  HrmSupervisor.decode_one_shot_argument!(encoded, "worker failure")
                )
              when "guard"
                capsule_path = ARGV.shift or raise HrmExperiment::ValidationError, HrmSupervisor.usage
                events_path = ARGV.shift or raise HrmExperiment::ValidationError, HrmSupervisor.usage
