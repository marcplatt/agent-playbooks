...
required_inputs: [execution_capsule, target_hrm_contract, repository_policy]
optional_inputs: [event_log, scorecard, approved_reference_requests]
...
```text
Run the target HRM under AP-EXEC-001/0.1.0-rc.1.

Execution capsule: {{execution_capsule}}
Target HRM contract: {{target_hrm_contract}}
...
2. Use the capsule as active-state authority. Conversation is not authoritative state.
   Emit an event for every lifecycle transition, material decision, candidate freeze,
   conclusive check, operator-review boundary, finding disposition, and terminal reason.
...
9. Select checks from changed reach plus binding repository rules. Use the exact checks,
   ...
   repeat a conclusive check for the same candidate, plan hash, and environment.
